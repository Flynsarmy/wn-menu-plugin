...
class Partial extends ItemTypeBase
{
     ...
     * @return string
     */
    public function onRender(MenuItem $item, Controller $controller, array $settings, $depth, $url, $child_count = 0)
    {
        ...
        return $controller->renderPartial($item->master_object_id, [
            ...
            'before_item' => sprintf(
                $settings['before_item'],
                $item->id,
                $item->id_attrib,
                $item->getClassAttrib($settings, $depth),
                $item->title_attrib
            ),
            'after_item'  => sprintf(
                $settings['after_item'],
